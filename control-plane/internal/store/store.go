...
const tenantCols = `id, name, coalesce(tenant_id,''), region, plan, enrollment, version,
	...
	cluster_mesh_installed, cluster_gateway_ip, cluster_ingress_issuer, cluster_mtls_strict, cluster_otel_installed`
...
func scanTenant(row pgx.Row) (model.Tenant, error) {
	...
	err := row.Scan(&t.ID, &t.Name, &t.TenantID, &t.Region, &t.Plan, &t.Enrollment,
		...
		&t.Cluster.MeshInstalled, &t.Cluster.GatewayIP, &t.Cluster.IngressIssuer, &t.Cluster.MTLSStrict, &t.Cluster.OTelInstalled)
	if installedAt != "" {
	...
	if c := hb.Cluster; c != nil {
		if _, err := s.pool.Exec(ctx,
			`UPDATE tenants SET cluster_name = $2, cluster_phase = $3, cluster_k8s_version = $4,
			   ...
			   cluster_mtls_strict = $11, cluster_otel_installed = $12
			 WHERE id = $1`,
			t.ID, c.Name, c.Phase, c.KubernetesVer, c.ArgoInstalled, c.NodeCount, c.Detail,
			c.MeshInstalled, c.GatewayIP, c.IngressIssuer, c.MTLSStrict, c.OTelInstalled); err != nil {
			return err
		}
	}
