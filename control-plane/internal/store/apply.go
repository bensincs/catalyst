...
func insertApplication(ctx context.Context, q querier, a model.Application, createdBy string) error {
	_, err := q.Exec(ctx,
		`INSERT INTO applications (id, name, description, owner_tenant, namespace, repo_url, chart, target_revision, values, expose_service, expose_port, hostname, oidc_scope, auth_required, embed, icon, wiring, dependencies, application_hooks, roles, created_by)
		...
		a.Embed, a.Icon,
		wiringJSON(a.Wiring), depsJSON(a.Dependencies), hooksJSON(a.ApplicationHooks), depsArray(a.Roles), createdBy)
	return err
}
