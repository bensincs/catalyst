...
func TestApplyWiring(t *testing.T) {
	...
	wiring := []shared.WireLink{
		{BicepOutput: "host", HelmPath: "database.host"},
		{BicepOutput: "port", HelmPath: "database.port"},
	}
	outputs := map[string]any{"host": "db.example.com", "port": float64(5432)}
	...
	bad := ": : not yaml"
	if applyWiring(bad, wiring, outputs) != bad {
